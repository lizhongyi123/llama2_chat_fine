...
def setup_environ_flags(rank):
    ...
    os.environ["TORCH_SHOW_CPP_STACKTRACES"] = str(1)  # 是否显示 PyTorch C++ 层面堆栈跟踪信息的标志
    os.environ["NCCL_ASYNC_ERROR_HANDLING"] = str(1)  # 是否启用 NCCL 异步错误处理。启用异步错误处理可以更好地处理 NCCL 操作中的错误。
...
def clear_gpu_cache(rank=None):
    ...
    # torch.cuda.empty_cache() 来清除所有 GPU 的缓存。
